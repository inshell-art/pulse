...
The public site is **https://pulse.inshell.art**, served by the Cloudflare Worker `pulse-site`. First published on 2026-09-26, with HTTPS routes and read-only Sepolia Core calculations verified. Website patch release: [`pulse-site-v1.0.1`](../../docs/site/release-v1.0.1.md). Local preview remains available for development.
...
The home page reads as one column: question → proposition → examination. Its opening states the supply–demand question; one highlighted equation connects the waiting time with the Price–Time Scale. The proposition’s continuous drop and PTS equations sit in its mathematics disclosure; the floor-dependent next-curve equation and integer calculations belong to the implementation section. The lab leads with one chart and a plain-language result; calculation details, rule settings, and purchase history are optional disclosures. The **Pulse is a DAA** section follows Core implementation directly, stating the commitment to public, fixed rules and accepting both success and failure as outcomes. It distinguishes that commitment from the guarantees of the stateless Core: each project must enforce it in its own application. The **Where Pulse fits** section follows DAA, distinguishing experimental art that makes the auction part of the work from studies of Pulse itself through simulations or separate auctions with different fixed settings. A compact project list follows, with a name, stage, and one-line description per project. The header jumps to this section; `/projects` retains the full summaries and evidence links. Both views use the same `projects.json` data. A compact **Build with Pulse** section follows the projects, explaining shared-Core calls and source-library imports, with links to integration instructions, the calculation source, and the Core V1 release. The header links to both sections; Build is the final section. A simulated purchase leaves the clock at the purchase instant so the immediate price jump is visible; the visitor moves time forward to make the next purchase.
...
The initial publication used the Worker custom-domain API after Wrangler uploaded the site: the deployment credential permitted custom domains but not the zone's Worker-route listing. The domain connection was checked for conflicts and added without replacing existing DNS records or services. For code and asset updates on that existing binding, use `npx wrangler@4.94.0 versions upload --tag pulse-site-v1.0.1`, then `npx wrangler@4.94.0 versions deploy <uploaded-version-id>@100% --yes`. Record the returned version ID and deploy that exact version; this flow leaves the existing domain binding in place. A full `wrangler deploy` also needs access to the zone's Worker routes.
