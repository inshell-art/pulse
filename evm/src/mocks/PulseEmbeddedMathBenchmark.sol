...
library PulseEmbeddedMathBenchmark {
    function initialize(IPulseCore.Config memory config, uint64 startTime)
        internal
        pure
    ...
    function quote(IPulseCore.Config memory config, IPulseCore.State memory state, uint64 timestamp)
        internal
        pure
        returns (uint256 ask)
    {
    ...
    function advance(IPulseCore.Config memory config, IPulseCore.State memory state, uint64 timestamp)
        internal
        pure
    ...
    function _validateConfig(IPulseCore.Config memory config) private pure {
    ...
    function _validateState(IPulseCore.Config memory config, IPulseCore.State memory state)
        private
        pure
    {
