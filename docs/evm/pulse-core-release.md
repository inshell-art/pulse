...
The chain-neutral manifest pins reviewed source and build contents by hash. The [Sepolia record](../../evm/releases/pulse-core-v1/sepolia.json) additionally names the Git commit containing that exact source and artifact set. The [`pulse-core-v1.0.0` release tag](https://github.com/inshell-art/pulse/releases/tag/pulse-core-v1.0.0) pins the complete distributable bundle. Keep the frozen build unchanged unless a new review deliberately replaces it. V1 core is immutable: a revised implementation needs a new address and reviewed release identity.
...
Run `npm run smoke:core:sepolia` from `evm/` for a repeatable, read-only live check. It uses `PULSE_RPC_URL` or the RPC setting in `~/.opsec/path/env/sepolia.env`; it does not load a signer, broadcast a transaction, or require the ignored deployment journals. It verifies the chain, frozen bytecode, version, deployment transaction and finality, then calls `initialize`, `quote` and `advance` against fixed expected results, including an integer-rounding case. This is a core smoke test; the two-consumer rehearsal below is the separate integration evidence.
...
Ethereum mainnet deployment is deferred. A read-only mainnet RPC quote estimated **647,046 gas** for creating the frozen core on 2026-09-23, but this is only a volatile historical snapshot and no production deployer was selected. The source commit and repository tag are published; explorer source publication remains separate and unapproved. Neither step grants mainnet deployment authorization.
